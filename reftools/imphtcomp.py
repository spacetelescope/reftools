"""Tools for comparing two IMPHTTAB tables from the same instrument and
detector.
...
__all__ = ['ImphtcompError', 'ImphttabComp', 'print_table_diffs',
           'plot_table_diffs']
...
class _ImphttabData:
    def __init__(self, data):
        ...
        for n in data.names:
            x = n.lower()
            if x not in ('pedigree', 'descrip'):
                self.names.append(x)
                self.__dict__[x] = data[n].copy()


class _ImphttabHolder:
    def __init__(self, fitshdu):
        self.photflam = _ImphttabData(fitshdu['PHOTFLAM'].data)
        self.photplam = _ImphttabData(fitshdu['PHOTPLAM'].data)
        self.photbw = _ImphttabData(fitshdu['PHOTBW'].data)


class ImphttabComp:
    """Class for comparing two IMPHTTAB tables from the same instrument and
    detector.
    ...
    def _read_tables(self, tab1, tab2):
        with fits.open(tab1, 'readonly') as fits1:
            with fits.open(tab2, 'readonly') as fits2:
                # check that some import header values match, especially
                # instrume/detector
                self._check_headers(fits1[0].header, fits2[0].header)

                # check that the two tables have at least some obsmodes in
                # common and report any differences
                self._check_obsmodes(
                    fits1[1].data['obsmode'], fits2[1].data['obsmode'])

                # we've made sure these tables can be compared, now read and
                # store the data.
                self.tab1 = _ImphttabHolder(fits1)
                self.tab2 = _ImphttabHolder(fits2)

    def _check_headers(self, head1, head2):
        """Check that some important header keywords match.
        An ImphtcompError is raised if they do not match.

        """
        # check that we've got an IMPHTTAB
        if 'DBTABLE' not in head1 or head1['dbtable'] != 'IMPHTTAB':
            raise ImphtcompError(
                f'{self.tab1_name} is not a valid IMPHTTAB table.')
        if 'DBTABLE' not in head2 or head2['dbtable'] != 'IMPHTTAB':
            raise ImphtcompError(
                f'{self.tab2_name} is not a valid IMPHTTAB table.')

        if head1['instrume'] != head2['instrume']:
            raise ImphtcompError(
              "IMPHTTAB tables are not for the same instrument. "
              f"File1: {head1['instrume']}, File2: {head2['instrume']}.")

        if head1['detector'] != head2['detector']:
            raise ImphtcompError(
              "IMPHTTAB tables are not for the same detector. "
              f"File1: {head1['detector']}, File2: {head2['detector']}.")
    ...
    def _check_obsmodes(self, obs1, obs2):
        ...
        Prints differing obsmodes and returns None if the tables appear
        consistent but have some differences.
        ...
        if only1:
            print(f'Modes appearing only in {self.tab1_name}:')
            for o in only1:
                print(f'\t{o}')
            print('')

        if only2:
            print(f'Modes appearing only in {self.tab2_name}:')
            for o in only2:
                print(f'\t{o}')
            print('')
    ...
    def _compare_tables(self):
        ...
        for i, mode in enumerate(flam1.obsmode):
            ...
            # there's a good chance the matching obsmode is on the same row in
            # both files so check for that to save calling where
            ...
            # one parameterized variable
                ...
                if flam1.nelem1[i] != flam2.nelem1[w]:
                    print(f"Mode {mode} does not have matching number of "
                          "parameterized elements in each file. "
                          f"File 1: {flam1.nelem1[i]} "
                          f"File 2: {flam2.nelem1[w]}")
                    continue

                # expand the obsmodes with parameterized variable values and
                # save the result for each expanded obsmode
                for j, f in enumerate(flam1.par1values[i]):
                    temp_mode = mode.replace(
                        flam1.par1names[i].lower(),
            ...
            # two parameterized variables
                ...
                if flam1.nelem1[i] != flam2.nelem1[w]:
                    print(f"Mode {mode} does not have matching number of "
                          "first parameterized elements in each file. "
                          f"File 1: {flam1.nelem1[i]} "
                          f"File 2: {flam2.nelem1[w]}")
                    continue
                ...
                if flam1.nelem2[i] != flam2.nelem2[w]:
                    print(f"Mode {mode} does not have matching number of "
                          "second parameterized elements in each file. "
                          f"File 1: {flam1.nelem2[i]} "
                          f"File 2: {flam2.nelem2[w]}")
                    continue
                ...
                for j in range(flam1.nelem2[i]):
                    for k in range(flam1.nelem1[i]):
                        temp_mode = mode.replace(
                            flam1.par1names[i].lower(),
                            flam1.par1names[i].lower() + str(flam1.par1values[k]))  # noqa
                        temp_mode = temp_mode.replace(
                            flam1.par2names[i].lower(),
                            flam1.par2names[i].lower() + str(flam1.par2values[j]))  # noqa

                        modes.append(temp_mode)
                        flams1.append(
                            flam1.photflam2[i][flam1.nelem2[i] * j + k])
                        flams2.append(
                            flam2.photflam2[w][flam1.nelem2[i] * j + k])
                        flamdiff.append((flams1[-1] - flams2[-1]) / flams1[-1])

                        plams1.append(
                            plam1.photplam2[i][flam1.nelem2[i] * j + k])
                        plams2.append(
                            plam2.photplam2[w][flam1.nelem2[i] * j + k])
                        plamdiff.append((plams1[-1] - plams2[-1]) / plams1[-1])
    ...
    def print_diffs(self, orderby='photflam', lines=25):
        ...
        parameter. Print the number of modes given in the lines parameter.
        ...
        orderby : {'photflam', 'photplam', 'photbw'}, optional
            The parameter by which to order the printed results, with modes
            having the largest absolute difference in this parameter printed at
            the top.
        ...
        if orderby.lower() == 'photflam':
            ...
            raise ImphtcompError(
                "Unrecognized orderby keyword. Must be one of "
                "'photflam','photplam','photbw'.")
        ...
        s = ('{:<45}{:<15.5e}{:<15.5e}{:< 15.8f}{:<15.5f}{:<15.5f}{:< 15.8f}'
             '{:<15.5f}{:<15.5f}{:< 15.8f}')
    ...
    def make_plot(self, outname='imphttab_comp.pdf'):
        ...
        fig.suptitle(f'{os.path.basename(self.tab1_name)}, '
                     f'{os.path.basename(self.tab2_name)}')
...
def print_table_diffs(table1, table2, orderby='photflam', lines=25):
    """Compare two IMPHTTAB tables and print their differences to the
    terminal. Print any obsmodes which are in either table but not in both.

    Also print the obsmodes and parameters for the modes which most differ
    in the parameter given in ``orderby``. This is for seeing which
    obsmodes have the largest percent difference in the specified
    parameter. Print the number of modes given in the ``lines`` parameter.
    ...
    orderby : {'photflam', 'photplam', 'photbw', 'all'}, optional
        The printed results are ordered according to the absolute
        difference in the specified parameter, with the mode with the
        largest absolute difference at the top of the list.
        Specifying ``'all'`` will print 3 tables, one ordered by each of
        the parameters.
